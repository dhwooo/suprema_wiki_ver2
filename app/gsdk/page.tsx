    ...
    <main className="min-h-screen bg-bg text-text">
      ...
      <div className="grid grid-cols-[250px_minmax(0,1fr)] max-[820px]:block">
        <aside className="sticky top-[74px] h-[calc(100vh-74px)] overflow-auto px-7 py-6 max-[820px]:static max-[820px]:h-auto max-[820px]:px-5 max-[820px]:py-3">
          <p className="mb-2 text-xs font-medium uppercase tracking-wide text-faint">GSDK</p>
          <h2 className="mb-2 text-[17px] font-medium text-secondary">Gateway SDK</h2>
        ...
        <section className="w-full max-w-[1540px] px-9 pb-20 pt-8 max-[820px]:px-5">
          <p className="mb-3 text-xs font-medium uppercase tracking-wide text-faint">Gateway SDK</p>
          <h1 className="mb-4 text-[42px] font-[560] leading-tight text-text max-[760px]:text-[32px]">GSDK</h1>
          ...
          <section className="mt-14 rounded-lg border border-edge bg-surface p-5">
            ...
            <div className="grid grid-cols-3 gap-3 max-[980px]:grid-cols-2 max-[640px]:grid-cols-1">
              {apiCollections.map((collection) => (
                <Link className="rounded-lg border border-edge bg-bg p-4 hover:border-edge-strong hover:bg-hover" href={`/api#coll-${collection.slug}`} key={collection.slug}>
                  <p className="mb-2 text-[17px] font-medium text-secondary">{collection.title}</p>
                  <p className="text-sm leading-6 text-muted">{collection.description}</p>
                  <Badge className="mt-4 h-6 px-2 text-xs">{collection.apis.length} APIs</Badge>
                </Link>
              ))}
            </div>
          </section>
...
function HubCard({
  ...
  const className = "block rounded-lg border border-edge bg-surface p-5 transition-colors hover:border-edge-strong hover:bg-hover";
  const content = (
    <>
