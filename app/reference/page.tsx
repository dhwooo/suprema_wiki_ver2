      ...
      <div className="grid grid-cols-[250px_minmax(0,1fr)] max-[820px]:block">
        <aside className="sticky top-[74px] h-[calc(100vh-74px)] overflow-auto px-7 py-6 max-[820px]:static max-[820px]:h-auto max-[820px]:px-5 max-[820px]:py-3">
          <div className="mb-7 max-[820px]:hidden">
            ...
            <h2 className="mb-2 text-[17px] font-medium leading-tight text-neutral-200">사용 예제</h2>
        ...
        <header className="mb-12">
          <p className="mb-3 text-xs font-medium uppercase tracking-wide text-neutral-600">Usage Reference</p>
          <h1 className="mb-4 text-[40px] font-[560] leading-tight text-neutral-100 max-[760px]:text-[32px]">G-SDK 사용 예</h1>
          ...
          <Link
            className="mt-6 inline-flex h-9 items-center gap-2 rounded-full border border-white/10 bg-neutral-900 px-3 text-sm font-medium text-neutral-300 hover:bg-white/8 hover:text-white"
            href="/api"
          >
          ...
          {docs.map((doc) => (
            <Link href={`/reference/${doc.slug}`} className="group block" key={doc.slug}>
              ...
              <div className="pt-5">
                ...
                <h2 className="mb-2 text-xl font-medium text-neutral-200 group-hover:text-white">{doc.title}</h2>
                <p className="text-[15px] leading-6 text-neutral-400">{doc.description}</p>
