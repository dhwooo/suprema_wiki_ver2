  ...
  return (
    <main className="min-h-screen bg-bg text-text">
      ...
      <div className="grid grid-cols-[250px_minmax(0,1fr)_250px] max-[1180px]:grid-cols-[250px_minmax(0,1fr)] max-[820px]:block">
        <aside className="sticky top-[74px] h-[calc(100vh-74px)] overflow-auto px-7 py-6 max-[820px]:static max-[820px]:h-auto max-[820px]:px-5 max-[820px]:py-3">
          <div className="mb-7 max-[820px]:hidden">
            ...
            <h2 className="mb-2 text-[17px] font-medium leading-tight text-secondary">{doc.category}</h2>
      ...
      <article className="w-full max-w-[1180px] px-9 pb-20 pt-8 max-[820px]:px-5 max-[820px]:pt-7">
        <div className="mb-8">
          <Link className="inline-flex h-9 items-center gap-2 rounded-full border border-edge bg-surface px-3 text-sm font-medium text-secondary hover:bg-hover hover:text-text" href="/api">
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_340px] gap-10 max-[960px]:grid-cols-1">
          <section className="scroll-mt-24" id="overview">
            ...
            <h1 className="mb-4 text-[42px] font-[560] leading-tight text-text max-[760px]:text-[32px]">{doc.title}</h1>
            <p className="mb-6 text-[18px] leading-8 text-muted">{doc.description}</p>
            ...
            <div className="space-y-9">
              {doc.sections.map((section) => (
                <section className="scroll-mt-24" id={`section-${tocId(section.title)}`} key={section.title}>
                  <h2 className="mb-3 text-2xl font-medium text-secondary">{section.title}</h2>
          ...
          <aside className="max-[860px]:order-first">
            ...
            <div className="scroll-mt-24 rounded-lg border border-edge bg-surface p-5" id="reference-info">
              <h2 className="mb-4 text-sm font-medium text-secondary">Reference 정보</h2>
              <dl className="space-y-3 text-sm">
                <div className="flex justify-between gap-4">
                  <dt className="text-muted">플랫폼</dt>
                  <dd className="text-secondary">{doc.platform}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-muted">워크플로</dt>
                  <dd className="text-secondary">{doc.workflow}</dd>
                </div>
                <div className="flex justify-between gap-4">
                  <dt className="text-muted">소요 시간</dt>
                  <dd className="text-secondary">{doc.time}</dd>
                </div>
              </dl>
            </div>
          </aside>
