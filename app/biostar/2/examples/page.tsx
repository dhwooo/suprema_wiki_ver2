...
export const metadata: Metadata = {
  ...
  description: "BioStar 2 API를 실제로 호출하는 cURL·JavaScript 코드 예제입니다.",
};

const API_BASE = "/biostar/2/api-reference";

export default function BioStar2ExamplesPage() {
  const scenarios = getExampleScenarios("bs2");
      ...
      <div className="grid grid-cols-[250px_minmax(0,1fr)_250px] max-[1180px]:grid-cols-[250px_minmax(0,1fr)] max-[820px]:block">
        ...
        <article className="w-full max-w-[1180px] px-9 pb-20 pt-8 max-[820px]:px-5">
          <section className="scroll-mt-24" id="overview">
            <p className="mb-3 text-xs font-medium uppercase tracking-wide text-faint">BioStar 2 Examples</p>
            <h1 className="mb-4 text-[42px] font-[560] leading-tight text-text max-[760px]:text-[32px]">BioStar 2 코드 예제</h1>
            <p className="max-w-3xl text-[17px] leading-8 text-muted">
              BioStar 2 API를 실제로 호출하는 코드입니다. 로그인으로 세션을 발급받아 <code className="rounded bg-active px-1.5 py-0.5 font-mono text-[14px]">bs-session-id</code> 헤더로 전달하고, 주요 리소스를 다루는 흐름을 cURL과 JavaScript(fetch)로 정리했습니다. 각 단계는 API Reference의 해당 endpoint로 연결됩니다.
            </p>
