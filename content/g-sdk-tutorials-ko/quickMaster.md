...
1. [마스터 게이트웨이를 설치하고 실행합니다]({{'/master/install/' | relative_url}}). [인증서 관리]({{'/master/certificate/' | relative_url}})에 설명된 대로 필요한 인증서를 생성합니다.
2. [장치 게이트웨이를 설치하고 실행합니다]({{'/gateway/install/' | relative_url}}). [설정]({{'/gateway/config/' | relative_url}}#master-gateway)에 설명된 대로 장치 게이트웨이가 마스터 게이트웨이에 연결되도록 구성합니다.
3. [Python 클라이언트 라이브러리를 다운로드합니다]({{'/python/install/' | relative_url}})
4. 인증서를 복사합니다.
   * 마스터 게이트웨이의 루트 인증서를 작업 디렉터리로 복사합니다. 기본적으로 인증서(_ca.crt_)는 마스터 게이트웨이 설치 디렉터리의 _cert_ 안에 있습니다.
   * 관리자 인증서와 그 개인 키를 작업 디렉터리로 복사합니다.
   * 테넌트 인증서와 그 개인 키를 작업 디렉터리로 복사합니다.
5. _example/quick/quick.py_ 의 서버 및 장치 정보를 필요에 맞게 변경합니다.
...
6. 실행합니다.
    ...
    데이터베이스를 초기화하려면 __-mi__ 옵션으로 한 번 실행해야 합니다. 데이터베이스 초기화에 대해서는 _initMasterGateway()_ 를 참고하세요.
    {: .notice--info}    

## 1. 개요

다음 단계로 G-SDK 서비스를 사용할 수 있습니다.

1. 게이트웨이에 연결하고 ___grpc.secure_channel___ 을 가져옵니다.
...
2. 마스터 게이트웨이에 로그인하고 이후 통신에 사용할 JWT 토큰을 가져옵니다.
    ```python
...
3. 채널을 사용하여 ___connect_master_pb2_grpc.ConnectMasterStub___ 과 같은 서비스 스텁을 생성합니다. 사용 가능한 서비스와 함수는 [API 레퍼런스]({{'/api/' | relative_url}})를 참고하세요.
   
    ```python
    stub = connect_master_pb2_grpc.ConnectMasterStub(channel)
    ```

4. 스텁을 사용하여 서비스의 함수를 호출합니다.
   
    ```python
    response = stub.Connect(connect_master_pb2.ConnectRequest(gatewayID=gatewayID, connectInfo=connInfo))    
    ```

_example_ 의 클래스는 해당 API의 사용법을 보여주기 위해 작성되었습니다. 실제 애플리케이션에서는 이러한 샘플 클래스를 사용할 필요가 없습니다.
{: .notice--warning}


## 2. 마스터 게이트웨이에 연결

가장 먼저 할 일은 마스터 게이트웨이에 연결하여 이후 통신에 사용할 ___grpc.secure_channel___ 을 가져오는 것입니다. 게이트웨이의 주소와 포트 번호를 알아야 합니다. 그리고 다음 인증서도 갖추고 있어야 합니다.

* 마스터 게이트웨이의 루트 CA 인증서
* 테넌트의 클라이언트 인증서와 그 키 파일
* 테넌트 생성과 같은 관리 작업의 경우, 관리자의 클라이언트 인증서와 그 키 파일
...
1. ___MasterClient___ 를 생성하고 게이트웨이에 연결합니다.
...
BioStar 장치와의 연결을 관리하는 방법에는 세 가지가 있습니다. 이 예제는 동기 API만 보여줍니다. 다른 API에 대해서는 [Connect Master API]({{'/api/connectMaster/' | relative_url}})와 [튜토리얼]({{'/python/connectMaster/' | relative_url}})을 참고하세요.
...
1. ___ConnectMasterSvc___ 를 생성합니다. 내부적으로 ___connect_master_pb2_grpc.ConnectMasterStub___ 을 만듭니다.
   
    ```python
    connectMasterSvc = ConnectMasterSvc(channel)
    ```

2. 지정한 장치에 연결합니다. 기본적으로 장치는 SSL을 사용하도록 설정되어 있지 않습니다. SSL을 사용하려면 먼저 [ConnectMaster.EnableSSL]({{'/api/connectMaster/' | relative_url}}#enablessl)을 사용하여 활성화해야 합니다. 반환된 장치 ID는 다른 API에서 사용됩니다.
...
[Device API]({{'/api/device/' | relative_url}})를 사용하면 지정한 장치의 정보를 가져올 수 있습니다.
...
[Finger API]({{'/api/finger/' | relative_url}})를 사용하면 지문을 스캔하고, 마지막으로 스캔된 이미지를 가져오며, 지문 옵션을 설정할 수 있습니다.
...
2. 장치에서 지문을 스캔하여 템플릿 데이터를 가져옵니다. 이 템플릿은 [User.SetFinger]({{'/api/user/' | relative_url }}#setfinger)를 사용하여 사용자에게 할당할 수 있습니다.
   
    ```python
    templateData = fingerSvc.scan(deviceID, finger_pb2.TEMPLATE_FORMAT_SUPREMA, QUALITY_THRESHOLD)
    ```

3. 스캔된 지문 이미지를 가져와 BMP 파일로 저장합니다.
...
[Card API]({{'/api/card/' | relative_url}})를 사용하면 카드를 스캔/기록하고, 블랙리스트를 관리하며, 카드 옵션을 설정할 수 있습니다.
...
3. BioStar 장치는 자격이 박탈된 카드를 비활성화하기 위해 블랙리스트를 관리합니다. 블랙리스트에 등록된 카드를 가져오거나 추가하거나 삭제할 수 있습니다.
...
[User API]({{'/api/user/' | relative_url}})를 사용하면 사용자를 가져오거나 등록하거나 삭제할 수 있습니다. 또한 사용자에게 지문/카드/그룹을 설정할 수 있습니다.
...
4. 사용자에게 지문을 설정합니다. 비슷한 방식으로 카드, 출입 그룹, 작업 코드(job code)도 설정할 수 있습니다.
...
[Event API]({{'/api/event/' | relative_url}})를 사용하면 장치에 저장된 이벤트 로그를 읽을 수 있습니다. 또한 모니터링을 활성화한 후 실시간 이벤트를 수신할 수 있습니다.
...
2. 이벤트 로그를 가져옵니다. 시작 ID와 반환받을 이벤트의 최대 개수를 지정할 수 있습니다.

    ```python
    events = eventSvc.getLog(deviceID, 0, MAX_NUM_OF_LOG)
    ```

3. JPG 형식의 이미지 로그를 가져옵니다. [CapabilityInfo.imageLogSupported]({{'/api/device/' | relative_url }}#CapabilityInfo)를 지원하는 장치만 이미지 로그를 저장할 수 있습니다. 또한 [Event.SetImageFilter]({{'/api/event/' | relative_url }}#setimagefilter)를 사용하여 이미지 로그를 저장할 이벤트 유형을 지정할 수 있습니다.
